...
def plot_late_lora(data):
    ...
    def assign_group(row):
        ...
        elif row['method'] == 'qlora' and row['update_matrices'] == 'standard':
            return 'QLoRA (Classic)'
        elif row['method'] == 'qlora' and row['update_matrices'] == 'all_matrices':
            return 'QLoRA (All)'
        else:
            return None
    ...
    fig, axes = plt.subplots(1, n_datasets, figsize=(15, 6), sharey=False)
    ...
    groups_order = ['LoRA (All)', 'LoRA (Classic)', 'QLoRA (All)', 'QLoRA (Classic)', 'Full Finetuning']
    ...
    for ax, dataset in zip(axes.flatten(), DATASETS):
        ...
        cluster_width = 0.6  # total width of the cluster
        ...
        for layer in unique_layers:
            ...
            for i, grp in enumerate(groups_order):
                ...
                for metric, val in sorted(metric_vals.items(), key=lambda item: item[1], reverse=True):
                    ax.bar(xpos, val, width=offsets[1]-offsets[0], color=CUSTOM_PALETTE[metric], alpha=1,
        ...
        metric_list_leg = ['ais', 'box', 'point']
        metric_handles = [Patch(facecolor=CUSTOM_PALETTE[m], label=m, alpha=0.7) for m in metric_list_leg]
